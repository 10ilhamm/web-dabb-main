...
<div class="feature-content" style="padding-bottom: 5rem;">
    <div class="container">
        ...
        <div style="background: #fff; border-radius: 12px; display: flex; gap: 10px; max-width: 1000px;">
            ...
            <div style="flex: 1;">
                <h2 style="font-size: 1.5rem; font-weight: 700; color: #222; margin: 5px 0 1rem;">Disclaimer</h2>
                
                <div style="font-size: 0.95rem; color: #555; line-height: 1.8;">
                    @php
                        $locale = app()->getLocale();
                        $key = $locale === 'en' ? 'disclaimer_content_en' : 'disclaimer_content';
                        $disclaimerContent = \App\Models\Setting::where('key', $key)->value('value');
                        
                        // Fallback ke bahasa Indonesia jika bahasa Inggris belum ada isinya
                        if (empty($disclaimerContent) && $locale === 'en') {
                            $disclaimerContent = \App\Models\Setting::where('key', 'disclaimer_content')->value('value');
                        }
                    @endphp
