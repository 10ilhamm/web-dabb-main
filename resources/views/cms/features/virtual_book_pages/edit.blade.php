...
<style>
    ...
    .preview-controls button:not(.active) {
        background: #e5e7eb;
        color: #374151;
    }
...
<form action="{{ route('cms.features.virtual_book_pages.update', [$feature, $virtualBookPage]) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    @csrf
    @method('PUT')
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <img src="{{ asset('storage/' . $virtualBookPage->image) }}" alt="Current Image" class="w-full h-auto rounded-lg border border-gray-200 mb-2" id="currentImagePreview">
                        <label class="flex items-center">
                            <input type="checkbox" name="remove_image" value="1" class="rounded border-gray-300" onchange="updatePreview()">
                            <span class="ml-2 text-sm text-gray-500">Hapus gambar</span>
                        </label>
                    </div>
...
<script>
    // Image sources
    let currentImageSrc = '{{ $virtualBookPage->image ? asset("storage/" . $virtualBookPage->image) : "" }}';
    let newImageSrc = '';
    ...
    function getImageSrc() {
        return newImageSrc || currentImageSrc;
    }
    ...
    function updatePreview() {
        ...
        const hasImage = imageSrc && !document.querySelector('input[name="remove_image"]')?.checked;
        ...
        if (pageType === 'cover') {
            ...
            const imageYPos = document.getElementById('imageYPosition').value;
            const contentYPos = document.getElementById('contentYPosition').value;
