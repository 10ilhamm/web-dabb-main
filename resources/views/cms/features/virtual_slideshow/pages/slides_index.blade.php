...
<div class="space-y-6" x-data="{ deleteModal: { open: false, id: null, name: '' } }">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800">🎞️ Kelola Slides: {{ $page->title }}</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
        <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
            <div>
                <h2 class="text-base font-semibold text-gray-800">Daftar Slide</h2>
                <p class="text-sm text-gray-500 mt-0.5">Atur urutan slide dan kelola konten interaktif.</p>
            </div>
        ...
        <div class="px-6 py-20 text-center">
            <div class="flex flex-col items-center gap-4">
                ...
                <p class="text-gray-400 text-sm">Belum ada slide. Klik "Tambah Slide" untuk memulai.</p>
                <a href="{{ route('cms.features.slideshow.pages.slides.create', [$feature, $page]) }}"
            ...
            <div class="flex items-center gap-4 px-6 py-4 hover:bg-gray-50/60 transition-colors">
                ...
                <div class="w-20 h-14 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0 flex items-center justify-center">
                    ...
                    @elseif($slide->images && count($slide->images) > 0)
                        <img src="{{ asset('storage/' . $slide->images[0]) }}" class="w-full h-full object-cover" alt="">
                    @else
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-1">
                        @php
                            $typeLabels = ['hero'=>'Hero','text'=>'Teks','carousel'=>'Carousel','video'=>'Video','text_carousel'=>'Teks + Carousel'];
                            $typeColors = ['hero'=>'bg-purple-100 text-purple-700','text'=>'bg-gray-100 text-gray-600','carousel'=>'bg-blue-100 text-blue-700','video'=>'bg-red-100 text-red-700','text_carousel'=>'bg-teal-100 text-teal-700'];
                        @endphp
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $typeColors[$slide->slide_type] ?? 'bg-gray-100 text-gray-600' }}">
                    ...
                    <p class="text-sm font-medium text-gray-800 truncate">{{ $slide->title ?: '(tanpa judul)' }}</p>
                    @if($slide->description)
                        <p class="text-xs text-gray-400 truncate mt-0.5">{{ Str::limit(strip_tags($slide->description), 80) }}</p>
                    @endif
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                    <a href="{{ route('cms.features.slideshow.pages.slides.edit', [$feature, $page, $slide]) }}"
                        class="inline-flex items-center justify-center w-8 h-8 bg-yellow-400 hover:bg-yellow-500 text-white rounded-md transition-colors" title="Edit">
        ...
        <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 rounded-b-xl">
            <a href="{{ url($feature->path) }}?page={{ $page->order }}" target="_blank"
                ...
                Lihat Halaman Publik (Exhibition #{{ $page->order }})
            </a>
        </div>
        @endif
    </div>
    ...
    <div x-show="deleteModal.open" x-cloak
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm z-[9999] p-6"
            ...
            <div class="flex flex-col items-center text-center gap-4">
                ...
                <div>
                    <h3 class="text-base font-semibold text-gray-800">Hapus Slide</h3>
                ...
                <div class="flex items-center gap-3 w-full">
                    ...
                    <form :action="`{{ url('cms/features/' . $feature->id . '/slideshow/pages/' . $page->id . '/slides') }}/${deleteModal.id}`" method="POST" class="flex-1">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
