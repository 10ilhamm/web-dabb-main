...
@section('breadcrumb_items')
    ...
    <span class="text-gray-300">/</span>
    <span class="text-gray-400">{{ __('cms.virtual_books.breadcrumb_edit') }}</span>
@endsection
...
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-800">{{ __('cms.virtual_books.edit_title', ['name' => $book->title]) }}</h1>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <label class="flex items-center">
                        ...
                        <span class="ml-2 text-sm text-gray-500">{{ __('cms.virtual_books.form.remove_cover') }}</span>
                    </label>
                    ...
                    <div id="additionalTextsContainer" class="space-y-2">
                        ...
                        @if(count($coverTextsArray) > 0)
                            ...
                            <div class="flex items-center gap-2" data-id="{{ $index }}">
                                <input type="text" name="cover_text_{{ $index }}" value="{{ $coverText['text'] ?? '' }}" placeholder="{{ __('cms.virtual_books.form.additional_text_placeholder', ['number' => $index + 1]) }}"
                ...
                <div>
                    ...
                    <input type="text" name="back_title" id="backBookTitle" value="{{ old('back_title', $book->back_title ?? '') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="{{ __('cms.virtual_books.form.back_title_placeholder') }}">
                </div>
                ...
                <div>
                    ...
                    <label class="flex items-center">
                        ...
                        <span class="ml-2 text-sm text-gray-500">{{ __('cms.virtual_books.form.remove_back_cover') }}</span>
                    </label>
                    <hr class="my-3 border-gray-200">
                    @endif
                ...
                <div>
                    ...
                    <div id="backAdditionalTextsContainer" class="space-y-2">
                        ...
                        @if(count($backCoverTextsArray) > 0)
                            ...
                            <div class="flex items-center gap-2" data-id="{{ $index }}">
                                <input type="text" name="back_cover_text_{{ $index }}" value="{{ $coverText['text'] ?? '' }}" placeholder="{{ __('cms.virtual_books.form.additional_text_placeholder', ['number' => $index + 1]) }}"
                ...
                <div>
                    ...
                    <label class="flex items-center">
                        ...
                        <span class="ml-2 text-sm text-gray-500">{{ __('cms.virtual_books.form.remove_thumbnail') }}</span>
                    </label>
                    <hr class="my-3 border-gray-200">
                    @endif
                    ...
                    <div id="thumbnailPreviewContainer" class="mt-2 hidden">
                        <p class="text-xs text-gray-500 mb-1">{{ __('cms.virtual_books.form.thumbnail_new_will_save') }}</p>
                        <img id="thumbnailPreview" class="w-24 h-32 object-cover rounded-lg border border-gray-200" alt="Thumbnail Preview">
                        <button type="button" id="removeThumbnail" class="mt-1 text-xs text-red-500 hover:text-red-700">{{ __('cms.virtual_books.form.cancel_remove') }}</button>
                    </div>
...
<script>
    ...
    window.bookEditorConfig = {
        ...
        additionalTexts: {!! json_encode(array_values(array_map(function($i, $ct) {
            return [
                'id' => $i,
                'text' => $ct['text'] ?? '',
                'position' => ['x' => $ct['position']['x'] ?? 0, 'y' => $ct['position']['y'] ?? 0]
            ];
        }, array_keys($initCoverTexts), $initCoverTexts))) !!},
        ...
        backAdditionalTexts: {!! json_encode(array_values(array_map(function($i, $bt) {
            return [
                'id' => $i,
                'text' => $bt['text'] ?? '',
                'position' => ['x' => $bt['position']['x'] ?? 0, 'y' => $bt['position']['y'] ?? 0]
            ];
        }, array_keys($initBackCoverTexts), $initBackCoverTexts))) !!}
    };
</script>
