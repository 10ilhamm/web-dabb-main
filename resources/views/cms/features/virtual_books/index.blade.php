...
<div class="space-y-6" x-data="{ deleteModal: { open: false, id: null, name: '' } }">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-800">{{ __('cms.virtual_books.page_title', ['name' => $feature->name]) }}</h1>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            ...
                            @else
                            <div class="w-16 h-20 bg-gray-100 rounded-md border border-gray-200 flex items-center justify-center text-xs text-gray-400">{{ __('cms.virtual_books.no_cover') }}</div>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <span class="text-sm text-gray-600">{{ __('cms.virtual_books.page_count', ['count' => $book->pages->count()]) }}</span>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <!-- Delete -->
                                <button @click="deleteModal = { open: true, id: {{ $book->id }}, name: '{{ addslashes($book->title) }}' }"
                                    class="inline-flex items-center justify-center w-8 h-8 bg-red-500 hover:bg-red-600 text-white rounded-md transition-colors" title="{{ __('cms.virtual_books.delete.title') }}">
            ...
            <div class="flex flex-col items-center text-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">
                        {{ __('cms.virtual_books.delete.confirm') }} <strong x-text="deleteModal.name" class="text-gray-700"></strong>{{ __('cms.virtual_books.delete.confirm_warn') }}
                    </p>
                </div>
                <div class="flex items-center gap-3 w-full">
                    ...
                    <form :action="`{{ route('cms.features.virtual_books.index', $feature) }}/${deleteModal.id}`" method="POST" class="flex-1">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
